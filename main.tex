...
\begin{document}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[node distance=2.2cm, auto]
			...
			\node (sw) [component, right=of browser, xshift=1.2cm] {\textit{[komponenta]} \\ Service Worker};
			\node (cache) [database, fill=yellow!10, below=of sw, yshift=-0.5cm] {\textit{[pohrana]} \\ IndexedDB};
			\node (nginx) [servernode, right=of sw, xshift=1.2cm] {\textit{[čvor]} \\ Nginx poslužitelj};
...
\end{lstlisting}
	...
	\section{ZAKLJUČAK I BUDUĆI RAD}
		...
		\begin{enumerate}
			...
			\item \textbf{Sučelje za predloške transakcija:} Implementacija sučelja za upravljanje predlošcima ponavljajućim transakcijama (engl. \textit{RecurringTemplate}). Time bi se korisnicima omogućilo lakše definiranje periodičkih troškova (npr. mjesečni najam, pretplate).
			
			\item \textbf{Automatsko prepoznavanje računa (OCR):} Integracija tehnologija računalnog vida i optičkog prepoznavanja znakova (engl. \textit{OCR}), čime bi se korisnicima omogućio unos fotografije računa, nakon čega bi sustav automatski izdvojio iznos, datum i naziv trgovca te ispunio formu za transakciju.
			
			\item \textbf{Integracija s otvorenim bankarstvom (PSD2):} Povezivanje aplikacije sa sustavima banaka putem PSD2 protokola, što bi omogućilo automatski uvoz i sinkronizaciju transakcija iz stvarnih bankovnih računa korisnika u račune u sustav Allocento.
