...
\begin{document}
	...
	Arhitektura sustava pomno je osmišljena kako bi podržala opisane zahtjeve. Pozadinski dio sustava (engl. \textit{backend}) temelji se na razvojnom okviru Laravel \cite{w_laravel} koji omogućuje izgradnju sigurnog i visoko skalabilnog REST API-ja te efikasno upravljanje robusnom relacijskom bazom podataka PostgreSQL \cite{w_postgresql}. Prednji dio sustava (engl. \textit{frontend}) implementiran je pomoću radnog okvira Angular \cite{w_angular}, čija napredna modularna arhitektura olakšava integraciju PWA modula, složeno upravljanje stanjima putem novog reaktivnog modela Signala te dinamičko lokalizirano formatiranje financijskih podataka. Vizualni identitet i konzistentnost korisničkog sučelja strogo su definirani primjenom dizajnerskih smjernica preuzetih iz platforme Logology \cite{w_logology}, čime se osigurava moderno i responzivno korisničko iskustvo na svim uređajima.
	...
	\section{ARHITEKTURA I DIZAJN SUSTAVA}
		...
		\begin{itemize}
			...
			\item \textbf{Ponavljajući predložak (\texttt{recurring\_templates}):} Pohranjuje predloške za transakcije koje se automatski ponavljaju pomoću pozadinskih poslova (CRON).
			\begin{itemize}
				\item \texttt{frequency} (Mjesečno, tjedno, godišnje)
				\item \texttt{day\_of\_month} (Dan u mjesecu kada se transakcija izvršava)
				\item \texttt{is\_active} (Status aktivnosti predloška)
			\end{itemize}
		\end{itemize}
	...
	\section{IMPLEMENTACIJA SUSTAVA "ALLOCENTO"}
		...
		Za osiguravanje privatnosti i autentičnosti pošiljatelja koristi se protokol \textbf{VAPID} (engl. \textit{Voluntary Application Server Identification}). Na poslužitelju se generira par ključeva: javni i privatni VAPID ključ.
...
\end{lstlisting}
      ...
      - name: Build frontend
		...
		\subsection{Osiguranje kvalitete i testiranje koda}
		Važan aspekt razvoja pouzdanih programskih rješenja je uspostava kvalitetnog procesa testiranja (engl. \textit{Quality Assurance} - QA). U sustavu Allocento, testiranje je prvenstveno fokusirano na pozadinski dio aplikacije, s obzirom na to da on obrađuje kritičnu poslovnu logiku, osigurava izolaciju radnih prostora (engl. \textit{tenant isolation}) te rukuje osjetljivim financijskim podacima.
		
		Testiranje u radnom okviru Laravel provodi se pomoću alata PHPUnit. Zbog upotrebe ugrađene SQLite memorijske baze podataka (kako je objašnjeno u arhitekturi baze), izvođenje stotina testova traje svega nekoliko sekundi, što omogućuje programerima vrlo brz povratni ciklus (engl. \textit{fast feedback loop}) prilikom pisanja novih funkcionalnosti \cite{w_laravel}.
		
		U sustavu su primijenjene dvije glavne razine testiranja:
		\begin{itemize}
			\item \textbf{Jedinično testiranje (engl. \textit{Unit testing}):} Provjerava ispravnost malih, izoliranih dijelova koda (obično pojedinačnih metoda unutar klasa), poput algoritama za formatiranje valuta, agregaciju statistika iz baze ili logiku izračuna datuma za ponavljajuće predloške transakcija.
			\item \textbf{Testiranje funkcionalnosti (engl. \textit{Feature testing}):} Oponaša stvarne klijentske HTTP zahtjeve usmjerene prema REST API-ju aplikacije, provjeravajući pritom ponašanje cijelog sustava (uključujući usmjerivač, \textit{middleware}, kontroler, modele i bazu podataka). Ovi testovi pokrivaju ključne korisničke scenarije poput pokušaja neovlaštenog pristupa podacima tuđeg radnog prostora, registracije s neispravnim podacima ili pokušaja negativnog iznosa u transakciji koja to ne podržava.
		\end{itemize}
		
		Svi testovi se automatski izvršavaju i na lokalnom računalu razvojnog inženjera, ali i kao integrirani korak unutar GitHub Actions CI/CD cjevovoda prije same izgradnje projekta, čime se sprječava da pogrešan (engl. \textit{buggy}) kod dospije u produkcijsko okruženje.
	...
	\section{KORISNIČKO SUČELJE I PRIKAZ RADA APLIKACIJE}
		...
		\subsubsection{Ponavljajući predlošci (Recurring Templates)}
		Za korisnike koji imaju fiksne mjesečne ili godišnje troškove (npr. najamnine, pretplate), sustav nudi sučelje za definiranje \textbf{ponavljajućih predložaka}. Korisnik može odrediti iznos, kategoriju, račun i frekvenciju ponavljanja. Sustav u pozadini (putem Cron poslova) automatski stvara nove transakcije kada nastupi zadani datum, čime se značajno smanjuje potreba za ručnim unosom i smanjuje se broj administrativnih pogrešaka.
		...
		\end{itemize}
		Prikaz uspješne sinkronizacije i primljene push obavijesti integriran je u korisničko iskustvo, čime se završava opis izvanmrežnog procesa.
